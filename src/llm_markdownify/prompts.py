...
MARKDOWN_SYSTEM_PROMPT = (
    ...
    "   - Use '# ' only when the line begins with a single integer section number followed by '.' (e.g., '1.', '12.') AND the remainder looks like a section name (short phrase, not a long sentence).\n"
    ...
    "   - If the content after the number is long/sentence-like (e.g., contains verbs such as 'shall', 'agree', 'will', 'must' early on, or clearly reads as a full clause), DO NOT make it a heading; keep it as a numbered line.\n"
    "\n"
    ...
    "\n"
    "Examples (Do/Don't):\n"
    "- Do: '# TERMS AND CONDITIONS' then '# 1. DEFINITIONS' then lines '1.1 …', '1.2 …'.\n"
    "- Do: '# 2. SCOPE OF ENGAGEMENT' (short, title-like).\n"
    "- Do: At page start with '4.3 …', keep it as a numbered line (continuation), DO NOT add '# 4' or '# 4.3'.\n"
...
MARKDOWN_USER_PROMPT = (
    ...
    "Apply the checklist strictly: '# ' only for concise, title-like integer-numbered sections ('1.', '12.'); decimal-numbered items never become headings (even if bold/uppercase/first line); if the text after an integer number reads as a long sentence, keep it as a numbered line; TOC entries are lists; '## ' reserved only for true non-numbered subheadings. \n"
    "If a table, boxed layout, or chart/figure continues across pages, MERGE those pages into one consistent representation."
)
